 ...
 *
 * Demonstrates different easing functions:
...
const EASING = {
  ...
  easeInOut: { type: "CUBIC_BEZIER" as const, x1: 0.42, y1: 0, x2: 0.58, y2: 1 },
  bounce: { type: "CUBIC_BEZIER" as const, x1: 0.68, y1: -0.55, x2: 0.265, y2: 1.55 },
};
...
creator.ui.onMessage((msg: Message) => {
  ...
  const createShape = (y: number, color: { r: number; g: number; b: number }) => {
  ...
  switch (msg.type) {
    case "compare-easings": {
      ...
      const shapes = [
        { shape: createShape(60, { r: 66, g: 133, b: 244 }), easing: EASING.linear, label: "Linear" },
        { shape: createShape(140, { r: 234, g: 67, b: 53 }), easing: EASING.easeIn, label: "Ease In" },
        { shape: createShape(220, { r: 251, g: 188, b: 5 }), easing: EASING.easeOut, label: "Ease Out" },
        { shape: createShape(300, { r: 52, g: 168, b: 83 }), easing: EASING.easeInOut, label: "Ease In Out" },
      ];
      ...
      shapes.forEach(({ shape, easing }) => {
        shape.position.addKeyframes([
          ...
          { frame: 60, value: { x: 350, y: shape.position.staticValue.y }, easing },
        ]);
      });
      break;
    }
