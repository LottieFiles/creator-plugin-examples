...
|---|---|---|
| **Runs** | Plugin code (`plugin.ts`) | Interface (`ui.html` or `App.tsx`) |
| **Has access to** | Creator Plugin APIs | Browser APIs |
| **Can** | Read/modify layers, shapes, keyframes; control timeline; access selection; store plugin data | Render UI with HTML or React; make network requests; handle user input |
...
The plugin sandbox listens with `creator.ui.onMessage()`. Messages arrive **unwrapped** (just the inner object). This is the same for both HTML and React plugins:
...
For React plugins with shared modules, define shared message types to prevent mismatches:
...
For HTML plugins, define an interface directly in `plugin.ts`:
...
**HTML plugins:** Use `<style>` tags or inline styles directly in `ui.html`.

**React plugins:** Any CSS approach — CSS files, Tailwind CSS, CSS modules, styled-components, CSS-in-JS.
